# Build and Deployment on NVIDIA DGX Spark (GB10, sm_121)

Guide. Target hardware: NVIDIA GB10 (DGX Spark) — compute capability 12.1
(`sm_121`), 48 SMs, unified LPDDR5X memory (~273 GB/s theoretical,
~220 GB/s sustained in GEMV-shaped reads; this number sets the decode
floor, see [QUANT.md](QUANT.md)).

The host needs only Docker with the NVIDIA runtime. All builds run inside
`nvidia/cuda:13.0.3-devel-ubuntu24.04` — no CUDA toolkit, Python, or
PyTorch on the host.
...
explicitly; the GEMM path is verified on `sm_121` (cos 0.9993 vs a BF16
cuBLAS reference). The fish-scales-ops *attention* kernels are
sm_120a-only and are not used.

## 2. Checkpoint and codec

```sh
scripts/fetch_model.sh model          # safetensors + tokenizer + config from HF
```

The DAC codec ships upstream as `codec.pth` (PyTorch). The native loader
reads a raw `codec.bin`/`codec.idx` pair instead (format documented in
...
(`s2p_dac_encode`); a decode-only artifact loads but disables encode. For
a cloning run of the test binary: `S2P_TEST_REF=<44.1 kHz mono s16 wav>`
plus `S2P_TEST_REF_TEXT=<transcript>` on `s2p-test`.
...
Prints per-stage timings and the first frames' codes, and writes
`/tmp/s2p_smoke.wav`. `S2P_INT8=1` selects the per-channel weight-only
INT8 path (parity-proven; halves decode time and weight memory);
`S2P_INT4=1` adds the packed group-wise INT4 backbone; `S2P_FP8=1`
selects the fish-scales-ops FP8 path (parity-failed, retained for
measurement only). See [QUANT.md §7](QUANT.md) for the full flag table.
...
## 6. Qualified nvcc flags (sm_121a)
...
## 7. fish-scales-ops integration notes
...
Weights are quantized once at load (128×128 block scale + SFB repack),
activations per call (1×128, UE8M0). Constraint: the activation-scale
repack requires `K % 512 == 0` — all S2-Pro GEMM widths qualify (2560,
4096, 6144, 9728); violations fall back to BF16 with a logged warning.
