"""Draft-model corpus: the big round. Full voice x text grid with fresh
seeds (7, 23, 51 — disjoint from the QAT rounds' 3/11/42, so no
duplicated deterministic trajectories even on shared combos), all 33
voices on the long passages too. Appends to the same dump file; run at
batch 1 (sequential requests) so the take sentinel cuts clean
sequences for the draft trainer."""
...
jobs = [("M", v, ti, s) for v in VOICES for ti in range(len(MEDIUM))
        for s in (7, 23, 51)]
random.Random(29).shuffle(jobs)
jobs = jobs[:1600]
longs = [("L", v, ti, s) for v in VOICES for ti in range(len(LONG))
         for s in (7, 23)]
random.Random(31).shuffle(longs)
jobs += longs[:100]
